...
- Preencher `projects/spec-template.md` com o estado real do projeto recém-criado
  (stack, status, regras de negócio, decisões). Isso vira o "cérebro" do projeto
  para sessões futuras.

## Árvore de decisão rápida (stack)
| Se o projeto é... | Stack sugerida (opções no catalog) |
|---|---|
| Web app pesado / CRM | Framework backend MVC + PostgreSQL + Redis |
